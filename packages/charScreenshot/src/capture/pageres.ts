...
import { captureWebsite, FileOptions, Options as CWOptions } from './captureWebsite';
...
export interface Options {
    ...
    /**
     ...
     @default {}
     */
    readonly launchOptions?: FileOptions['launchOptions'];
}
 ...
 */
export class PageRes extends EventEmitter {
        ...
        await Promise.all(this.src().map(async (source: Source): Promise<void> => {
            ...
            const screenshots = await pMap(
                ...
                async (size: string): Promise<Screenshot> => {
                    ...
                    return PageRes.create(source.url, size, options);
                },
    ...
    private async resolution(url: string, options: Options): Promise<void> {
        for (const item of await getResolutionsMemoized() as Array<{ item: string }>) {
            ...
            this.items.push(await PageRes.create(url, item.item, options));
        }
    }
    ...
    private async viewport(viewport: Viewport, options: Options): Promise<void> {
        ...
        for (const size of arrayUniq(viewport.sizes)) {
            this.items.push(await PageRes.create(viewport.url, size, options));
        }
    }
    ...
    private static async create(url: string, size: string, options: Options): Promise<Screenshot> {
