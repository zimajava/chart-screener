...
const settingsJson = fs.readFileSync(settingsPath, {encoding: 'utf8'})
...
const anychartExport = require('./anychart-node')(anychart);
...
app.get('/old/:tid/:assetName/:timeframe', async (req: Request, res: Response) => {
...
app.get('/:tid/:assetName/:timeframe', async (req: Request, res: Response) => {
    ...
    let rawData
    try {
        const { data } = await axios.get(`https://informer.investforum.ru/wss/api/quotation/${pair.ID}/${period}/1000/?withCurrentBar=true`)
    ...
    // map the data
    const mapping = dataTable.mapAs({'open': "O", 'high': "H", 'low': "L", 'close': "C"});
    ...
    try {
        ...
        const picture = anychartExport.exportToSync(chart, 'jpg')
...
// app.get('/cjs', async (req: Request, res: Response) => {
//     const start = Date.now()
//
//     let rawData
//     try {
//         const { data } = await axios.get('https://informer.investforum.ru/wss/api/quotation/23/Minute/1000/?withCurrentBar=true')
//         rawData = data
//     } catch (e) {
//         console.error(e)
//     }
...
// app.get('/am', async (req: Request, res: Response) => {
//     const start = Date.now()
//
//     const amChartPath = isProd ? resolve('amchart') : resolve('../../amchart')
//
//     // const am4core = require(`${amChartPath}/core`);
//     // const am4charts = require(`${amChartPath}/charts`);
//     // const am4themes_animated = require(`${amChartPath}/themes/animated`);
...
//
//     chart.data = [
//         {"date": "2011-08-01", "open": "136.65", "high": "136.96", "low": "134.15", "close": "136.49"},
//         {
//             "date": "2011-08-02",
//             "open": "135.26",
//             "high": "135.95",
...
//
//     try {
//         const picture = await chart.exporting.getImage("png");
//
//         res.status(200).set('Content-type', 'image/png').send(picture)
//         console.log(`Completed (${Date.now() - start} ms)`)
//     } catch (e) {
