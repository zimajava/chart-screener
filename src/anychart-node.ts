...
const defaultFontsDir = path.resolve(process.cwd(), 'fonts');
...
function loadExternalResources(resources, params, callback) {
  if (Object.prototype.toString.call(resources) === '[object Array]') {
    ...
    for (let i = 0, len = resources.length; i < len; i++) {
      request.get(resources[i], function (err, response, body) {
        if (err) {
          // @ts-ignore
...
function getSvgString(svgElement, width, height) {
  let svg = '';
  if (svgElement) {
    if (!width || isPercent(width)) width = defaultBounds.width;
    if (!height || isPercent(height)) height = defaultBounds.height;
...
function getSvg(target, params, callback) {
  ...
  if (dataType === 'svg') {
    ...
    if (params.document) prepareDocumentForRender(params.document);
    ...
    return loadExternalResources(res, params, function (err, params) {
      ...
      if (dataType === 'javascript') {
        const script = new vm.VM({
          timeout: 10000,
          sandbox: { anychart: window.anychart },
        });
        ...
        if (isChart || isStage) {
          if (target.animation) target.animation(false);
          if (target.a11y) target.a11y(false);
